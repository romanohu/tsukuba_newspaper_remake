...
def search_bm25(
    ...
    *,
    ...
    for r, (doc_id, score) in enumerate(ranked, start=1):
        ...
        row = {
            "rank": r, "doc_id": doc_id, "ext_id": ext_id, "title": title,
            "score": float(score), "length": dl
        }
        if include_path: row["path"] = path
        if return_terms: row["hit_terms"] = sorted(doc_hit_terms.get(doc_id, []))
        results.append(row)
    return results
...
def rrf_fuse(bm25_scores: Dict[int, float], vec_scores: Dict[int, float], k: int = 60) -> Dict[int, float]:
    b_rank = {d: r for r, (d, _) in enumerate(sorted(bm25_scores.items(), key=lambda x: x[1], reverse=True), start=1)}
    v_rank = {d: r for r, (d, _) in enumerate(sorted(vec_scores.items(), key=lambda x: x[1], reverse=True), start=1)}
    docs = set(b_rank) | set(v_rank)
    return {d: 1.0/(k + b_rank.get(d, 10**9)) + 1.0/(k + v_rank.get(d, 10**9)) for d in docs}

def weighted_sum_fuse(bm25_scores: Dict[int, float], vec_scores: Dict[int, float], w_bm25=0.6, w_vec=0.4) -> Dict[int, float]:
    ...
    B = norm(bm25_scores); V = norm(vec_scores)
    docs = set(B) | set(V)
    ...
    *,
    ...
    if q_terms:
        ...
        if term_rows:
            ...
            docid_to_len = {doc_id: length for doc_id, (_, _, _, length) in meta_docs.items()}
            bm25_scores = _bm25_rank(posts, termid_to_idf, docid_to_len, k1, b, avgdl)
            bm25_scores = dict(sorted(bm25_scores.items(), key=lambda x: x[1], reverse=True)[:bm25_k])


    vec_hits = search_vec(faiss_index_path, meta_json_path, query, model_name=model_name, topk=vec_k)
